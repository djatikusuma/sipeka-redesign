...
@section('content')
    <div class="card mb-5 mb-xl-8">
        <div class="card-body">
            <form class="form w-100" id="form_create_user" action="{{ route('users.store') }}" method="POST">
                @csrf
                @method('POST')
                <x-field-input label-name="Nama Lengkap" field-name="name" placeholder="Nama Lengkap" />
                <x-field-input label-name="Email" field-name="email" placeholder="Email" />
                ...
                <div class="fv-row mb-10">
                    ...
                    <select class="form-control" name="unors" id="unors_select">
                        @foreach ($unors as $unor)
                            <option value="{{ $unor->id }}">{{ $unor->unor_name }}</option>
                        @endforeach
                    </select>
                    @error('unors')
                        <div class="fv-plugins-message-container invalid-feedback">
